...
import os
from .models import Products,Carts,CartItems,Address,Orders,OrderItems
...
class ProductListView(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self):
        ...
        product_name=self.request.GET.get("pro")
...
class InputAddressView(LoginRequiredMixin,CreateView):
    ...
    def get(self,request,pk=None):
        cart=get_object_or_404(Carts,user_id=request.user.pk)
    ...
    def get_context_data(self,**kwargs):
        context=super().get_context_data(**kwargs)
        address=cache.get(f'address_user_{self.request.user.pk}')
        pk=self.kwargs.get("pk")
        address= get_object_or_404(Address,user_id=self.request.user.pk,pk=pk)if pk else address
        if address:
        ...
        context["addresses"]=Address.objects.filter(user=self.request.user).all()
        return context
...
class ConfirmOrderView(LoginRequiredMixin,TemplateView):
    ...
    def get_context_data(self, **kwargs) :
        ...
        cart=get_object_or_404(Carts,user_id=self.request.user.pk)
        context["cart"]=cart
        total_price=0
        items=[]
